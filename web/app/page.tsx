...
export default async function Home({
  ...
  return (
    <div className="space-y-8">
      <section className="overflow-hidden rounded-3xl border border-line bg-white px-6 py-8 md:px-10">
        <p className="text-sm font-medium text-yes-deep">Prediction markets · settled in Macho Bucks</p>
        ...
        <div className="grid gap-3 md:grid-cols-4">
          {featured.map((market) => (
            <Link
              ...
              className="rounded-2xl border border-line bg-white p-4 hover:border-yes"
            >
              <div className="text-xs text-slate-500">{market.event.category}</div>
              <div className="mt-1 line-clamp-2 min-h-12 font-medium">{market.title}</div>
              <div className="mt-4 flex items-center justify-between text-sm">
                <span className="font-semibold text-yes-deep">Yes {formatCents(market.yes_price_cents)}</span>
                <span className="text-slate-500">{formatVolume(market.volume_macho_bucks)}</span>
              </div>
            </Link>
          ))}
        </div>
          ...
          {categories.map((item) => (
            <Link
              ...
              className={`rounded-full border px-3 py-1.5 text-sm ${
                cat === item ? "border-transparent bg-no text-white" : "border-line bg-white"
              }`}
            >
              {item}
            </Link>
