...
import type { MarketView } from "@/lib/cowshi/types";
import { Sparkline } from "./charts";

export function MarketTable({ markets }: { markets: MarketView[] }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-line bg-surface">
      <div className="hidden grid-cols-[1.6fr_90px_90px_110px_120px_90px] gap-3 border-b border-line px-4 py-2 text-[11px] font-medium uppercase tracking-wide text-muted md:grid">
        <div>Market</div>
        <div>Yes</div>
        <div>No</div>
        <div>Volume</div>
      ...
      {markets.length === 0 ? (
        <div className="px-4 py-8 text-sm text-muted">No markets match that filter.</div>
      ) : (
        markets.map((market) => (
          <Link
            key={market.ticker}
          ...
          >
            <div>
              ...
              <div className="font-mono text-xs text-muted-2">{market.ticker}</div>
            </div>
            <PriceChip side="yes" cents={market.yes_price_cents} />
            <PriceChip side="no" cents={market.no_price_cents} />
            <div className="text-sm text-muted tabular-nums">{formatVolume(market.volume_macho_bucks)}</div>
...
export function PriceChip({ side, cents }: { side: "yes" | "no"; cents: number }) {
  ...
  return (
    <div
      className={`inline-flex w-full items-center justify-between rounded-lg px-2 py-1.5 text-sm font-semibold tabular-nums ${
        yes ? "bg-yes-soft text-yes-text" : "bg-no-soft text-no-text"
      }`}
    >
