//#region node_modules/.nitro/vite/services/ssr/assets/engine-DMoMN_5H.js
function nextId(store) {
	store.next_id += 1;
	return store.next_id;
}
function aggregateBook(orders, ticker, side) {
	const levels = /* @__PURE__ */ new Map();
	for (const order of orders) {
		if (order.ticker !== ticker || order.side !== side || order.status !== "open" || order.remaining <= 0) continue;
		levels.set(order.price_cents, (levels.get(order.price_cents) ?? 0) + order.remaining);
	}
	const sorted = [...levels.entries()].map(([price_cents, size]) => ({
		price_cents,
		size
	}));
...
function bumpAverage(prevQty, prevAvg, addQty, price) {
	const total = prevQty + addQty;
	if (total <= 0) return 0;
	return (prevQty * prevAvg + addQty * price) / total;
}
function creditPosition(store, userId, ticker, side, qty, price) {
...
function debitPosition(store, userId, ticker, side, qty) {
...
function matchBuy(store, userId, ticker, side, limitCents, quantity) {
	...
	if (!Number.isInteger(quantity) || quantity < 1) throw new Error("Contracts must be a whole number, at least 1");
	if (!Number.isInteger(limitCents) || limitCents < 1 || limitCents > 99) throw new Error("Price must be a whole number of cents between 1¢ and 99¢");
	const opposite = side === "yes" ? "no" : "yes";
	let remaining = quantity;
	let spent = 0;
	const now = (/* @__PURE__ */ new Date()).toISOString();
	const candidates = store.orders.filter((o) => o.ticker === ticker && o.side === opposite && o.status === "open" && o.remaining > 0 && o.user_id !== userId && o.price_cents >= 100 - limitCents).sort((a, b) => b.price_cents - a.price_cents || a.id - b.id);
	...
	return {
		...
		spent,
		last: market.yes_price_cents
	};
}
function matchSell(store, userId, ticker, side, limitCents, quantity) {
	const market = store.markets.find((m) => m.ticker === ticker);
	const user = store.users.find((u) => u.id === userId);
	if (!market || !user) throw new Error("Unknown market or user");
	if (market.status !== "open") throw new Error("Market is not open");
	...
	let proceeds = 0;
	const now = (/* @__PURE__ */ new Date()).toISOString();
	const candidates = store.orders.filter((o) => o.ticker === ticker && o.side === side && o.status === "open" && o.remaining > 0 && o.user_id !== userId && o.price_cents >= limitCents).sort((a, b) => b.price_cents - a.price_cents || a.id - b.id);
	for (const rest of candidates) {
		if (remaining <= 0) break;
		const fill = Math.min(remaining, rest.remaining);
...
function resolveMarket(store, ticker, outcome) {
	const market = store.markets.find((m) => m.ticker === ticker);
	if (!market) throw new Error("Unknown market");
	if (market.status === "resolved") throw new Error("Already resolved");
	for (const order of store.orders) if (order.ticker === ticker && order.status === "open") {
...
//#endregion
export { aggregateBook, lastChange, matchBuy, matchSell, nextId, resolveMarket };
